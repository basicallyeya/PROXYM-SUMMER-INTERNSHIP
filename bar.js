...
This folder is a voice+screen "brief" recorded with the QAhelper browser extension.
...
async function handleRecordingFinished({ recordingB64, mimeType, durationMs, transcriptFinal, transcriptChunks, keyframes }) {
  ...
  const filename = `qahelper/brief-${briefId}.zip`;
...
async function finishWithCopy(id, intent) {
  const prompt =
    `Process brief ${id} from ~/Downloads/qahelper/: ` +

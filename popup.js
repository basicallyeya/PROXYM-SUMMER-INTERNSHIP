...
// ---------- Add a ticket ----------
// Find the smallest unused "Ticket N" so anonymous additions don't collide.
function nextAutoBriefName(list) {
  const used = new Set();
  for (const b of list) {
    const m = (b?.name || '').trim().match(/^ticket\s+(\d+)$/i);
...
addForm.addEventListener('submit', async (e) => {
  e.preventDefault();
  const list = await getInbox();
  // Empty input → auto-name "Ticket N" instead of bailing. The user clearly
...
function briefReadme(b) {
  const parts = [];
  parts.push('# Brief\n');
  parts.push('A brief captured with the QAhelper extension. Read skill/SKILL.md and follow it to file a tracker ticket.\n');
  parts.push('## Contents');
...
async function writeBriefZip(b) {
  ...
  const res = await chrome.runtime.sendMessage({
    type: 'DOWNLOAD_ZIP',
    payload: { blobUrl, filename: `qahelper/brief-${b.id}.zip` },
  });
...
async function writeCompanionZip(b) {
  ...
  const res = await chrome.runtime.sendMessage({
    type: 'DOWNLOAD_ZIP',
    payload: { blobUrl, filename: `qahelper/brief-${b.id}-extra.zip` },
  });
  URL.revokeObjectURL(blobUrl);
...
exportBtn.addEventListener('click', async () => {
  ...
  const prompt =
    `Process briefs from ~/Downloads/qahelper/: ${named}. ` +
  ...
  try {
    ...
    if (guidance) {
      ...
      await chrome.runtime.sendMessage({
        type: 'DOWNLOAD_ZIP',
        payload: { blobUrl, filename: 'qahelper/guidance.txt' },
      });
      URL.revokeObjectURL(blobUrl);
    }
  } catch {}

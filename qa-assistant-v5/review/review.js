...
function showQualityIndicator(data) {
  ...
  const hasMic = data.transcript && data.transcript.length > 0;
  const micStatus = hasMic
    ? '<span class="quality-item quality-ok">🎤 Mic: ✓ Audio captured</span>'
    : '<span class="quality-item quality-warn">🎤 Mic: No audio detected</span>';
  div.innerHTML = `
    <span class="quality-item">⏱ ${fmt(dur)}</span>
    <span class="quality-item">&nbsp;·&nbsp;</span>
    <span class="quality-item">${fmtSize(size)}</span>
    <span class="quality-item">&nbsp;·&nbsp;</span>
    <span class="quality-item">${data.extension ? data.extension.toUpperCase() : 'MP4'}</span>
    <span class="quality-item">&nbsp;·&nbsp;</span>
    ${micStatus}`;
  // Insert after stats row
  const statsRow = document.querySelector('.stats-row');
  if (statsRow && statsRow.parentNode) statsRow.parentNode.insertBefore(div, statsRow.nextSibling);
}

/* ── Severity counter ── */
function updateSeverityCounter() {
  const counter = document.getElementById('severity-counter');
  if (!counter) return;
  const anns = window.__qa_annotations__ || [];
  if (anns.length === 0) { counter.style.display = 'none'; return; }
  const counts = { critical:0, high:0, medium:0, low:0 };
  anns.forEach(a => { if (counts[a.severity] !== undefined) counts[a.severity]++; });
  counter.style.display = 'flex';
  ['critical','high','medium','low'].forEach(sev => {
    const el = document.getElementById('sev-' + sev);
    if (el) el.querySelector('.sev-num').textContent = counts[sev];
    if (el) el.style.opacity = counts[sev] > 0 ? '1' : '0.35';
  });
}
...
chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
  if (msg.type === 'RECORDING_DATA' && msg.data) {
    ...
    showQualityIndicator(recordingData);
...
function renderTranscriptList(segments, source, sourceLabel) {
  ...
  segments.forEach(e => {
    ...
    row.innerHTML = `<span class="t-ts t-ts-link" title="Click to seek to ${fmt(e.ts)}">${fmt(e.ts)}</span><span class="t-text"></span>`;
    row.querySelector('.t-text').textContent = text;
    row.querySelector('.t-ts').addEventListener('click', () => {
      const video = document.getElementById('rv');
      if (video && isFinite(e.ts)) { video.currentTime = e.ts; video.play().catch(()=>{}); }
    });
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const dlBtn2 = document.getElementById('btn-dl');
  if (dlBtn2) dlBtn2.addEventListener('click', () => { downloaded = true; }, { capture: true });
  window.addEventListener('beforeunload', (e) => {
    if (blobURL && !downloaded) {
      e.preventDefault();
      e.returnValue = 'You have an unsaved recording. Download it before closing to prevent data loss.';
...
/* ════════════════════════════════
   EXPORT PDF
   Uses window.print() with the review-print.css stylesheet.
   Chrome's print dialog lets the user save as PDF.
════════════════════════════════ */
function exportPDF() {
  // Temporarily remove max-height from transcript list so all entries print
  const tList = document.querySelector('.t-list');
  let oldMaxH = '';
  if (tList) { oldMaxH = tList.style.maxHeight; tList.style.maxHeight = 'none'; }

  // Add print-only title block
  const printHeader = document.createElement('div');
  printHeader.id    = '__qa_print_header__';
  printHeader.style.cssText = 'display:none;';
  printHeader.className = 'qa-no-print'; // will be shown by print CSS
  document.body.appendChild(printHeader);

  window.print();

  // Restore
  if (tList) tList.style.maxHeight = oldMaxH;
  printHeader.remove();
}

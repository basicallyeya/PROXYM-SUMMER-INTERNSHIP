...
/* ── Session Templates ── */
const TEMPLATES = {
  login:       { layer:'Frontend', testType:'Functional', environment:'Staging',
    rules:['Verify login with valid credentials','Test invalid password shows error','Test empty fields validation','Test forgot password link','Check remember me option','Test login on mobile viewport'] },
  checkout:    { layer:'Frontend', testType:'Functional', environment:'Staging',
    rules:['Add item to cart','Apply discount code','Fill shipping details','Test invalid card number','Complete payment flow','Verify order confirmation email'] },
  api:         { layer:'API', testType:'Functional', environment:'Development',
    rules:['Check endpoint returns 200 OK','Test with invalid token returns 401','Verify response schema matches docs','Test rate limiting behavior','Check error messages are descriptive','Test pagination parameters'] },
  mobile:      { layer:'Mobile', testType:'Functional', environment:'Staging',
    rules:['Test on 375px viewport','Verify touch targets are large enough','Check horizontal scroll is absent','Test landscape orientation','Verify images scale correctly','Check font sizes are readable'] },
  regression:  { layer:'Full Stack', testType:'Regression', environment:'Staging',
    rules:['Verify all previous bugs are fixed','Test core user flows end to end','Check no new regressions introduced','Validate database consistency','Test third party integrations'] },
  smoke:       { layer:'Full Stack', testType:'Smoke', environment:'Production',
    rules:['App loads without errors','User can log in','Core navigation works','Main feature is accessible','No critical console errors'] },
  accessibility:{ layer:'Frontend', testType:'Functional', environment:'Staging',
    rules:['All images have alt text','Keyboard navigation works','Color contrast meets WCAG AA','Screen reader announces elements','Focus indicators are visible','Forms have proper labels'] },
  performance: { layer:'Full Stack', testType:'Functional', environment:'Production',
    rules:['Page load under 3 seconds','No layout shift on load','Images are optimized','API response under 500ms','Check for memory leaks','Lighthouse score above 80'] },
};
...
/* ── Session Template ── */
const templateSel = document.getElementById('session-template');
if (templateSel) {
  templateSel.addEventListener('change', () => {
    ...
    if (t.layer)       { document.getElementById('ctx-layer').value = t.layer;       state.layer       = t.layer; }
    if (t.testType)    { document.getElementById('ctx-type').value  = t.testType;    state.testType    = t.testType; }
    if (t.environment) { document.getElementById('ctx-env').value   = t.environment; state.environment = t.environment; }
    // Apply rules
    state.rules = [...t.rules];
    renderRules();
    toast('Template applied: ' + templateSel.options[templateSel.selectedIndex].text.replace(/^.+? /, ''));
  });
}
...
function renderSessionCard(s) {
  ...
  card.innerHTML = `
    <div class="session-card-head">
      ...
      <div style="display:flex;align-items:center;gap:6px;">
        ...
        <button class="btn-session-del" title="Delete session">
          <svg viewBox="0 0 20 20" fill="none" width="14" height="14">
            <path d="M4 6h12M8 6V4h4v2M7 6l.5 9.5a1 1 0 001 .95h3a1 1 0 001-.95L13 6" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"/>
          </svg>
        </button>
      </div>
    </div>
  ...
  card.querySelector('.btn-session-del').addEventListener('click', (e) => {
    e.stopPropagation();
    // Show inline confirmation bar on the card
    const existing = card.querySelector('.del-confirm-bar');
    if (existing) { existing.remove(); return; }
    const bar = document.createElement('div');
    bar.className = 'del-confirm-bar';
    bar.innerHTML = `
      <span class="del-confirm-msg">Delete this session?</span>
      <button class="del-confirm-yes">Delete</button>
      <button class="del-confirm-no">Cancel</button>
    `;
    card.appendChild(bar);
    bar.querySelector('.del-confirm-no').addEventListener('click', (e) => {
      e.stopPropagation(); bar.remove();
    });
    bar.querySelector('.del-confirm-yes').addEventListener('click', async (e) => {
      e.stopPropagation();
      try {
        await qaDeleteSession(s.id);
        card.remove();
      ...
      } catch(err) { toast('Could not delete session'); }
    });
  });
  return card;
}
...
async function loadResults() {
  ...
  if (!sessions || sessions.length === 0) {
    empty.style.display = ''; data.style.display = 'none'; return;
  }
  ...
  if (stats) {
    ...
    function fmtMin(s) { return s < 60 ? s + 's' : Math.floor(s/60) + 'm ' + (s%60) + 's'; }
    stats.innerHTML = `<span>📁 <strong>${total}</strong> session${total!==1?'s':''}</span>
      <span>⏱ <strong>${fmtMin(totalSec)}</strong> total</span>
      <span>∅ <strong>${fmtMin(avgSec)}</strong> avg</span>`;
  }
  ...
  function renderFiltered(query) {
    ...
    const filtered = q
      ? sessions.filter(s => (s.title||'').toLowerCase().includes(q) || (s.tester||'').toLowerCase().includes(q))
      : sessions;
    if (filtered.length === 0) {
      list.innerHTML = `<p style="font-size:12px;color:var(--text-secondary);text-align:center;padding:16px 0;">No sessions match "${q}"</p>`;
      return;
    }
    filtered.forEach(s => list.appendChild(renderSessionCard(s)));
  }
  ...
  if (search) {
    search.value = '';
    search.oninput = () => renderFiltered(search.value);
  }
}

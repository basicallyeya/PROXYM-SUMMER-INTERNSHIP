...
chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
  ...
  /* Screenshot from control bar → capture tab → send to recorder */
  if (msg.type === 'TAKE_SCREENSHOT') {
    // Use a short delay to allow the controlbar to hide first
    setTimeout(() => {
      chrome.tabs.query({ active: true, currentWindow: true }, tabs => {
        if (!tabs[0]) return;
        chrome.tabs.captureVisibleTab(tabs[0].windowId, { format: 'png' }, (dataUrl) => {
          if (chrome.runtime.lastError || !dataUrl) return;
          // Store in chrome.storage so review page can retrieve it
          chrome.storage.session.get(['qa_screenshots'], (r) => {
            const shots = r.qa_screenshots || [];
            shots.push({ dataUrl, timestamp: msg.timestamp });
            chrome.storage.session.set({ qa_screenshots: shots });
          });
          // Also relay to recorder tab if open
          chrome.tabs.query({}, allTabs => {
            const rec = allTabs.find(t => t.url && t.url.includes('recorder/recorder.html'));
            if (rec) {
              chrome.tabs.sendMessage(rec.id, {
                type: 'SCREENSHOT_TAKEN', dataUrl, timestamp: msg.timestamp
              }).catch(() => {});
            }
          });
        });
    ...
    sendResponse({ ok: true }); // respond immediately before async
    return false;
  }
